...
Records live under the cascade's own directory, per `/manager-mode`'s canonical layout. `TEST-AUDIT-BRIEF.md` is built exactly as base 3.4.1 describes — `build_audit_brief.py` compiles the umbrella test, shard's stated goal + brief set, spec excerpt, tests under audit, `BOUNDARIES.md` and composition-relevant contract excerpts; the overlord adds only the sibling-shard-awareness and already-litigated sections by hand.
