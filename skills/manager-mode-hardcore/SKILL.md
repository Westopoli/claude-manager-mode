...
Read `/manager-mode` first and execute its Phases 0–7 unchanged EXCEPT Phase 3.4, which this file replaces: preflight, lite-discovery, the blocking Phase 1.5 plan-consistency pass, shard-test-writer per-leaf RED tests, the rest of Phase 3's invariant audit (non-overlap, no-design, no-contradiction, sizing, spec-link, codebase-preconditions), dependency map + consolidation pass + 16-leaf sizing limit, parent ownership, the Phase 4.0 wave baseline and per-leaf sandboxes, parallel builder dispatch, sandbox harvest + sweep, questions/proposals, bypass detection and its per-leaf gate evidence, G1–G10, file-match, umbrella pre/post checks, admit-or-revert, apex testing, and reporting.
